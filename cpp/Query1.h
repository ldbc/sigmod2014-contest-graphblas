...
class Query1 : public Query<uint64_t, uint64_t, int> {
    ...
    GrB_Index componentSize(GrB_Matrix A, GrB_Index start) {
        GBxx_Object<GrB_Vector> levels = GB(LAGraph_bfs_pushpull, nullptr, A, A, start, -1, true);
        ok(GxB_Vector_select(levels.get(), GrB_NULL, GrB_NULL, GxB_GT_ZERO, levels.get(), GrB_NULL, GrB_NULL));
        return GBxx_nvals(levels);
    }
    ...
    std::tuple<std::string, std::string> initial_calculation() override {
        ...
        add_comment_if_on("p1ComponentSize", std::to_string(componentSize(A, p1)));
        add_comment_if_on("p2ComponentSize", std::to_string(componentSize(A, p2)));
